...
export const crearPet = async (req, res) => {
    try {
        ...
        if (resultado.affectedRows > 0) {
            res.status(200).json({ message: 'Pet registrado con éxito' });
        } else {
            res.status(404).json({ message: 'No se pudo registrar el Pet' });
        }
    } catch (error) {
...
export const listarPet = async (req, res) => {
  try {
    const [pets] = await pool
      .promise()
      .query(
        "SELECT nombre, pets.id, races.name AS race_name, categories.name AS category_name, pets.photo, genders.name AS gender_name FROM pets JOIN races ON pets.race_id = races.id JOIN categories ON pets.category_id = categories.id JOIN genders ON pets.gender_id = genders.id;"
      );
    if (pets.length > 0) {
      res.status(200).json({ pets });
    } else {
      res.status(404).json({ message: "No hay pets registrados" });
    }
  } catch (error) {
    res.status(500).json({ message: "Error del servidor: " + error.message });
  }
...
export const buscarPet2 = async(req,res)=>{
  try {
    const {id} =req.params
    const [pets] = await pool.promise().query('SELECT pets.id, pets.nombre, races.id AS race_id, races.name AS race_name, categories.id AS category_id, categories.name AS category_name, pets.photo, genders.id AS gender_id, genders.name AS gender_name FROM pets JOIN races ON pets.race_id = races.id JOIN categories ON pets.category_id = categories.id JOIN genders ON pets.gender_id = genders.id WHERE pets.id=?',[id])
    if (pets.length > 0) {
      res.status(200).json({ pets });
    } else {
...
export const actualizarPet = async (req, res) => {
  try {
    const { id } = req.params;
    const { Nombre, race_id, category_id, gender_id } = req.body;
...
export const buscarpet = async(req,res) =>{
  try {
    const {id} = req.params
    const [resultado] = await pool.promise().query('SELECT pets.nombre, pets.id, races.name AS race_name, categories.name AS category_name, pets.photo, genders.name AS gender_name FROM pets JOIN races ON pets.race_id = races.id JOIN categories ON pets.category_id = categories.id JOIN genders ON pets.gender_id = genders.id WHERE pets.id = ?',[id])
