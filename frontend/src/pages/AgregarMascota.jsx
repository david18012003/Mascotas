...
function AgregarMascotas() {
  ...
  const ListarGenero = async () => {
    const url = 'http://localhost:3000/gender/listar';
    try {
  ...
  const ListarCategoria = async () => {
    const url = 'http://localhost:3000/categorias/listar';
    try {
  ...
  const ListarRaza = async () => {
    const url = 'http://localhost:3000/race/listar';
    try {
      const response = await axios.get(url,{header:{token:token}});
      if (response.status === 200) {
  ...
  const handleFileChange = (e) => {
    ...
    setImagePreview(URL.createObjectURL(file)); // Crear una URL de objeto para la vista previa
  };
